...
function isDaylightSavings(date, year) {
  const startDST = new Date(`${year}-03-29`); // Last Sunday of March
  const endDST   = new Date(`${year}-10-25`); // Last Sunday of October
  return date >= startDST && date < endDST;
}

function filterWorkoutsByYear(workoutsByDate, year) {
  return Object.fromEntries(
    Object.entries(workoutsByDate).filter(([date]) =>
...
const container = document.getElementById("calendar-table");
  ...
  const leftdates = [
    `${selectedYear}-01-02`,`${selectedYear}-02-02`,`${selectedYear}-03-02`,`${selectedYear}-04-02`,
    `${selectedYear}-05-02`,`${selectedYear}-06-02`,`${selectedYear}-07-02`,`${selectedYear}-08-02`,
    `${selectedYear}-09-02`,`${selectedYear}-10-02`,`${selectedYear}-11-02`,`${selectedYear}-12-02`
  ];

  const topdates = [
    `${selectedYear}-01-07`,`${selectedYear}-01-08`,
    `${selectedYear}-02-02`,`${selectedYear}-02-03`,`${selectedYear}-02-04`,`${selectedYear}-02-05`,`${selectedYear}-02-06`,`${selectedYear}-02-07`,`${selectedYear}-02-08`,
    `${selectedYear}-03-02`,`${selectedYear}-03-03`,`${selectedYear}-03-04`,`${selectedYear}-03-05`,`${selectedYear}-03-06`,`${selectedYear}-03-07`,`${selectedYear}-03-08`,
    `${selectedYear}-04-02`,`${selectedYear}-04-03`,`${selectedYear}-04-04`,`${selectedYear}-04-05`,`${selectedYear}-04-06`,`${selectedYear}-04-07`,`${selectedYear}-04-08`,
    `${selectedYear}-05-02`,`${selectedYear}-05-03`,`${selectedYear}-05-04`,`${selectedYear}-05-05`,`${selectedYear}-05-06`,`${selectedYear}-05-07`,`${selectedYear}-05-08`,
    `${selectedYear}-06-02`,`${selectedYear}-06-03`,`${selectedYear}-06-04`,`${selectedYear}-06-05`,`${selectedYear}-06-06`,`${selectedYear}-06-07`,`${selectedYear}-06-08`,
    `${selectedYear}-07-02`,`${selectedYear}-07-03`,`${selectedYear}-07-04`,`${selectedYear}-07-05`,`${selectedYear}-07-06`,`${selectedYear}-07-07`,`${selectedYear}-07-08`,
    `${selectedYear}-08-02`,`${selectedYear}-08-03`,`${selectedYear}-08-04`,`${selectedYear}-08-05`,`${selectedYear}-08-06`,`${selectedYear}-08-07`,`${selectedYear}-08-08`,
    `${selectedYear}-09-02`,`${selectedYear}-09-03`,`${selectedYear}-09-04`,`${selectedYear}-09-05`,`${selectedYear}-09-06`,`${selectedYear}-09-07`,`${selectedYear}-09-08`,
    `${selectedYear}-10-02`,`${selectedYear}-10-03`,`${selectedYear}-10-04`,`${selectedYear}-10-05`,`${selectedYear}-10-06`,`${selectedYear}-10-07`,`${selectedYear}-10-08`,
    `${selectedYear}-11-02`,`${selectedYear}-11-03`,`${selectedYear}-11-04`,`${selectedYear}-11-05`,`${selectedYear}-11-06`,`${selectedYear}-11-07`,`${selectedYear}-11-08`,
    `${selectedYear}-12-02`,`${selectedYear}-12-03`,`${selectedYear}-12-04`,`${selectedYear}-12-05`,`${selectedYear}-12-06`,`${selectedYear}-12-07`,`${selectedYear}-12-08`
  ];
  ...
  topdates.forEach(date => {
    customBorders[date] = customBorders[date]
  ...
  for (let i = 1; i <= 52; i++) {
    ...
    for (let day = 1; day <= 7; day++) {
      const currentDate = new Date(`${selectedYear}-01-01`);
      currentDate.setDate(currentDate.getDate() + (i - 1) * 7 + (day - 1));
      currentDate.setDate(currentDate.getDate() - 1);

      if (isDaylightSavings(currentDate, selectedYear)) {
        currentDate.setDate(currentDate.getDate() + 1);
      }
      ...
      if (dateKey === today.toISOString().split('T')[0] && !workoutData) {
        color = 'rgb(255, 255, 146)';
      } else if (currentDate < today && !workoutData) {
        color = 'rgb(233, 233, 233)';
      } else if (workoutData) {
        if (workoutData.type === 'Running') {
          color = 'rgb(139, 160, 243)';
        } else if (workoutData.type === 'Activity') {
          color = getactivityColor(workoutData.maxIntensity);
        } else {
      ...
      const customBorderStyle = customBorders[dateKey] || "";
...
// Re-render calendar when year changes
document.getElementById('year-selector').addEventListener('change', createCalendar);
